...
class DoctorDetailsCubit extends Cubit<DoctorDetailsState> {
  ...
  Future<void> addReview(Review review) async {
    setLoading();
    final response = await _doctorRepository.addReview(
        review, state.selectedDoctor!.rating ?? 0);
    response.fold((l) => setError(l.message), (r) {
      setappointmentSuccess(doctor: r);
    });
  }
  ...
  Future<void> getDoctorAppointmentsAtDate(
    ...
    setLoading();
    final response =
        await _doctorRepository.getDoctorAppointmentsAtDate(doctorId, date);
    response.fold((l) {
      setError(l.message);
    }, (r) {
      emit(state.copyWith(
          status: DoctorDetailsStatus.getDoctorAppointmentsAtDate,
          doctorAppointmentsAtDate: r));
    });
  }
  ...
  void setappointmentSuccess({DoctorModel? doctor}) {
    emit(state.copyWith(
        status: DoctorDetailsStatus.appointmentAdded, selectedDoctor: doctor));
  }

  void setAppointmentSuccess({DoctorModel? doctor}) {
    emit(state.copyWith(
        status: DoctorDetailsStatus.success, selectedDoctor: doctor));
  }
}
